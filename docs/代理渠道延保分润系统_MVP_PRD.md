...
| 登录后跳转  | 代理商账户首页                         |
| 协议入口    | 登录页展示“登录即代表同意用户协议与隐私政策”；“用户协议”“隐私政策”均可点击打开对应协议页面 |
...
- 运营点击“通过”前，系统校验代理商已完成实名认证且已完成电子签约。
- 任一未完成则拦截并提示。
- 驳回时必须填写原因，原因写入记录并下发微信通知给代理商。
- 通过后流程：审核通过 → 下发“审核通过”微信通知 → 灵工平台打款 → 打款成功后下发“提现到账”通知。
- 提现通过后，对应已提现订单不可线上退款。
...
| 情形         | 判断条件                        | 处理方式                                      |
| ---------- | --------------------------- | ----------------------------------------- |
| 不参与分润      | 订单所属渠道未配置分润规则，或订单类型不触发分润    | 弹确认弹窗，确认后直接退款；退款信息同步至小程序；代理可提现余额不变        |
| 参与分润 · 未提现 | 订单已产生分润入账记录，但对应分润仍在代理可提现余额中 | 弹确认弹窗；确认后直接退款，同时扣减代理可提现余额，小程序分润流水新增退款扣回记录 |
| 参与分润 · 已提现 | 订单分润已通过提现申请，并已打款至代理银行账户     | 不可自动退款；弹提示弹窗，告知运营线下通知代理商处理                |
